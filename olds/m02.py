...
class sun:
    def __init__(self, d, obs_loc=None):
        ...
        self.x_equ = self.x_ecl
        self.y_equ = self.y_ecl * cos(ecl*rd) - self.z_ecl * sin(ecl*rd)
        self.z_equ = self.y_ecl * sin(ecl*rd) + self.z_ecl * cos(ecl*rd)

        self.R   =  sqrt(self.x_equ**2 + self.y_equ**2 + self.z_equ**2)
        self.RA  =  arctan2(self.y_equ, self.x_equ)*(180/pi)
        self.DEC =  arctan2(self.z_equ, sqrt(self.x_equ**2 + self.y_equ**2))*(180/pi)
...
class moon:
    ...
    def __init__(self, d, obs_loc=None):
        ...
        ecl_lon =  rev(arctan2(y_ecl, x_ecl)*(180/pi)) # (MOHEM: rev)
        ecl_lat =  arctan2(z_ecl, sqrt(x_ecl**2 + y_ecl**2))*(180/pi)
        ecl_r   =  sqrt(x_ecl**2 + y_ecl**2 + z_ecl**2)
        ...
        x_equ = x_ecl
        y_equ = y_ecl * cos(ecl*rd) - z_ecl * sin(ecl*rd)
        z_equ = y_ecl * sin(ecl*rd) + z_ecl * cos(ecl*rd)

        self.ra  = rev(arctan2(y_equ, x_equ)*(180/pi)) # MOHEM: rev
        self.dec = arctan2(z_equ, sqrt(x_equ**2 + y_equ**2))*(180/pi)
...
class planet:
    ...
    def __init__(self, name, d, obs_loc=None, epoch=None):
        ...
        self.lon_h_ecl =  rev(arctan2(yh_ecl, xh_ecl)*(180/pi)) # (MOHEM: rev)
        self.lat_h_ecl =  arctan2(zh_ecl, sqrt(xh_ecl**2 + yh_ecl**2))*(180/pi)
        self.r_h_ecl   =  sqrt(xh_ecl**2 + yh_ecl**2 + zh_ecl**2)
        ...
        # kh: hypatie
        self.xh_ecl = self.r_h_ecl * cos(self.lat_h_ecl*rd) * cos(self.lon_h_ecl*rd)
        self.yh_ecl = self.r_h_ecl * cos(self.lat_h_ecl*rd) * sin(self.lon_h_ecl*rd)
        self.zh_ecl = self.r_h_ecl * sin(self.lat_h_ecl*rd)

        self.xg_ecl = self._sun.x_ecl + self.xh_ecl
        self.yg_ecl = self._sun.y_ecl + self.yh_ecl
        self.zg_ecl = self._sun.z_ecl + self.zh_ecl

        self.lon_g_ecl =  rev(arctan2(self.yg_ecl, self.xg_ecl)*(180/pi))
        self.lat_g_ecl =  arctan2(self.zg_ecl, sqrt(self.xg_ecl**2 + self.yg_ecl**2))*(180/pi)
        self.r_g_ecl = sqrt(self.xg_ecl**2 + self.yg_ecl**2 + self.zg_ecl**2)

        # equatorial rectangular geocentric
        self.x = self.xg_ecl
        self.y = self.yg_ecl * cos(ecl*rd) - self.zg_ecl * sin(ecl*rd)
        self.z = self.yg_ecl * sin(ecl*rd) + self.zg_ecl * cos(ecl*rd)

        # converet to spherical coordinates (radec)
        self.r = sqrt(self.x**2 + self.y**2 + self.z**2)
        self.ra  = rev(arctan2(self.y, self.x)*(180/pi))
        self.dec = arctan2(self.z, sqrt(self.x**2 + self.y**2))*(180/pi)
